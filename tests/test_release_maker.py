...
from releasemanager import eap_versions, existing_tags, mac_versions, ReleaseManager, str2bool, Version, latest_minor, batch_of_versions
...
def test_slice_job_short():
    ...
    assert batch_of_versions(versions, 0, 12) == [versions[0]]
    assert batch_of_versions(versions, 1, 12) == [versions[1]]
    assert batch_of_versions(versions, 2, 12) == [versions[2]]
    assert batch_of_versions(versions, 3, 12) == [versions[3]]
    assert batch_of_versions(versions, 4, 12) == [versions[4]]

    assert batch_of_versions(versions, 9, 12) == []
    assert batch_of_versions(versions, 5, 12) == []
    assert batch_of_versions(versions, 11, 12) == []

def test_slice_job_long():
     versions = [f"3.2.{i}" for i in range(68)]
     assert batch_of_versions(versions, 0, 12) == versions[0:6]
     assert batch_of_versions(versions, 1, 12) == versions[6:12]
     assert batch_of_versions(versions, 11, 12) == versions[63:68]

     processed = []
     for off in range(12):
         processed += batch_of_versions(versions, off, 12)
...
def test_slice_includes_all_versions():
    ...
    assert batch_of_versions(versions, 0, 8) == versions[0:2]
    assert batch_of_versions(versions, 1, 8) == versions[2:4]
    assert batch_of_versions(versions, 2, 8) == versions[4:6]
    assert batch_of_versions(versions, 3, 8) == versions[6:7]
    assert batch_of_versions(versions, 4, 8) == versions[7:8]
    assert batch_of_versions(versions, 5, 8) == versions[8:9]
    assert batch_of_versions(versions, 6, 8) == versions[9:10]
    assert batch_of_versions(versions, 7, 8) == versions[10:11]
